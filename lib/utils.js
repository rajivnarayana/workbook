...
exports.HLOOKUP = function (needle, table, index, exactmatch) {
    ...
    for (var i = 0; i < row.length; i++) {
        ...
        if ((exactmatch && row[i] === needle) ||
            row[i] && row[i].toLowerCase().indexOf(needle.toLowerCase()) !== -1) {
...
exports.VLOOKUP = function (needle, table, index, exactmatch) {
    if (exports.ISRANGE(table)) {
        table = table.valueOf();
        return exports.HLOOKUP(needle, table.map((row, rowIndex) => row.map((col, colIndex) => table[colIndex][rowIndex])), index, exactmatch);
    }
    return error.na;
}
