...
function workbook(config) {
    ...
    Object.defineProperty(this.constructor, "name", { get: function () { return "workbook"; } })
...
workbook.prototype.populateGraph = function (dep, f) {
    ...
    var filteredPrecedents = precedents.filter(function (precedent, index) {
        return Array.from(precedents).splice(index + 1).every(function (otherPrecedent) {
            return fn.ISRANGE(precedent) || !(
    ...
    filteredPrecedents.forEach(function(precedent) { this.graph.push([dep, precedent])}.bind(this));
...
workbook.prototype.setFormula = function (sheetIndex, ref, exp) {
    ...
    if (fn.ISCELL(result)) return +result;
    return result;
}
// ```
...
workbook.prototype.update = function (sheetIndex, cell) {
    ...
    if (dirtyCells) {
        for (var i = 0; i < dirtyCells.length; i++) {
            ...
            var oldValue = this.getValue(sheetIndex, rowIndex, colIndex);
            var newValue = this.recalculate(sheetIndex, cellIndex);
